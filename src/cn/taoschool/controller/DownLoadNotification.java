...
public class DownLoadNotification {
	String TAG = DownLoadNotification.class.getName();
	...
	public void showCustomizeNotification(int icoId,String titleStr,int layoutId) {  
        ...
        if(remoteView==null)
        {
            ...
            remoteView.setImageViewResource(R.id.iv_download,icoId);  
            remoteView.setTextViewText(R.id.tv_detail, titleStr); 
    ...
    public void changeProgressStatus(int p)
    {
        ...
        {
            if(p==DownLoadAPKThread.DOWNLOAD_FAIL)
                notification.contentView.setTextViewText(R.id.tv_download , "下载失败！ "); 
            else if(p==100)
                notification.contentView.setTextViewText(R.id.tv_download , "下载完成，请点击安装"); 
            else               
                notification.contentView.setTextViewText(R.id.tv_download , "进度("+p+"%) : "); 
            notification.contentView.setProgressBar(R.id.pb_download, 100, p, false);
        }
        notificationManager.notify(notificationID, notification);
    }
